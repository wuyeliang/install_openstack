...
SECONF_ETH=`ip addr | grep ^2: |awk -F ":" '{print$2}' | awk -F " " '{print$1}'`
...
SECONF_ETH=`ip addr | grep ^2: |awk -F ":" '{print$2}' | awk -F " " '{print$1}'`
ifconfig ${SECONF_ETH}:1  ${SECOND_NET}
ovs-vsctl add-port br-ex ${SECONF_ETH}:1  &&  ethtool -K ${SECONF_ETH}:1 gro off
fn_log "ovs-vsctl add-port br-ex ${SECONF_ETH}:1  &&  ethtool -K ${SECONF_ETH}:1 gro off"

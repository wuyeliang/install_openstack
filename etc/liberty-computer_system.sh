...
if [ -f  /etc/openstack-liberty_tag/presystem-computer.tag ]
then 
...
then
	mkdir  /etc/openstack-liberty_tag
fi

echo `date "+%Y-%m-%d %H:%M:%S"` >/etc/openstack-liberty_tag/presystem-computer.tag

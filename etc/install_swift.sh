...
cat $PWD/lib/proxy-server.conf >/etc/swift/proxy-server.conf
fn_log "cat $PWD/proxy-server.conf >/etc/swift/proxy-server.conf"
cat <<END >/tmp/tmp
...
cat $PWD/lib/account-server.conf >   /etc/swift/account-server.conf
fn_log "cat $PWD/lib/account-server.conf >   /etc/swift/account-server.conf"
cat $PWD/lib/container-server.conf >  /etc/swift/container-server.conf
fn_log "cat $PWD/lib/container-server.conf >  /etc/swift/container-server.conf"
cat $PWD/lib/object-server.conf >  /etc/swift/object-server.conf
fn_log "cat $PWD/lib/object-server.conf >  /etc/swift/object-server.conf"
...
cat $PWD/lib/swift.conf >/etc/swift/swift.conf
fn_log "cat $PWD/lib/swift.conf >/etc/swift/swift.conf"
cat <<END >/tmp/tmp
...
OBJECT_NODE=`cat $PWD/lib/swiftrc | grep '\[' | awk -F '[' '{print$2}' | awk -F ']' '{print$1}'`
fn_log "OBJECT_NODE=`cat $PWD/lib/swiftrc | grep '\[' | awk -F '[' '{print$2}' | awk -F ']' '{print$1}'`"
echo " "
SWIFT_DISK='sdc sdd'
#SWIFT_DISK=`cat $PWD/lib/swiftrc | grep -A 1  ${HOSTNAME}  | grep -v  ${HOSTNAME}`
fn_log "SWIFT_DISK=`cat $PWD/lib/swiftrc | grep -A 1  ${HOSTNAME}  | grep -v  ${HOSTNAME}`"
echo  ${SWIFT_DISK}
